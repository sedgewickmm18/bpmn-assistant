...
from bpmn_assistant.core.enums.models import FireworksAIModels
...
class FireworksAIProvider(LLMProvider):
    ...
    def check_model_compatibility(self, model: str) -> bool:
        return model in [m.value for m in FireworksAIModels]
        ...
        if self.output_mode == OutputMode.JSON:
            try:
                ...
                if not isinstance(result, dict):
                    raise ValueError(
                        f"Invalid JSON response from Fireworks AI: {result}"
                    )
                ...
                raise Exception("Invalid JSON response from Fireworks AI") from e
